    ...
    return (
        <div className="w-full bg-[#0d1117] rounded-3xl border border-white/10 overflow-hidden shadow-2xl font-mono transition-all duration-500 hover:shadow-[#1fa3de]/10 hover:border-[#1fa3de]/30">
            ...
            <div className="flex flex-col lg:flex-row min-h-[400px] lg:h-[550px]">
                ...
                {/* Editor Area */}
                <div className="flex-grow flex flex-col bg-[#0d1117]/50 backdrop-blur-xl overflow-hidden">
                    {/* Tabs */}
                    <div className="flex bg-[#161b22] overflow-x-auto no-scrollbar scrollbar-hide">
                        {files.map(file => (
                            <button
                                key={file.id}
                                onClick={() => setActiveFile(file)}
                                className={`px-4 py-3 text-[9px] lg:text-[10px] font-black flex items-center gap-2.5 border-r border-white/5 whitespace-nowrap transition-all ${activeFile.id === file.id ? 'bg-[#0d1117] text-[#1fa3de] border-t-2 border-t-[#1fa3de]' : 'text-white/40 hover:text-white/60 hover:bg-white/5'}`}
                            >
                                <span style={{ color: file.color }} className="font-black text-[8px]">{file.icon}</span>
                                {file.id}
                            </button>
                        ))}
                    </div>
                    ...
                    {/* Content View */}
                    <div className="p-4 lg:p-10 overflow-y-auto flex-grow relative custom-scrollbar">
                        <div className="absolute left-0 top-0 bottom-0 w-6 lg:w-14 bg-white/[0.01] border-r border-white/5 flex flex-col items-center py-4 lg:py-10 text-[8px] lg:text-[11px] text-white/20 select-none font-mono">
                            {Array.from({ length: 25 }).map((_, i) => <div key={i} className="h-5 lg:h-6 leading-5 lg:leading-6">{i + 1}</div>)}
                        </div>
                        
                        <div className="ml-3 lg:ml-10 space-y-6 lg:space-y-8">
                            <AnimatePresence mode="wait">
                                ...
                                >
                                    <div className="text-[11px] lg:text-sm leading-5 lg:leading-6">
                                        ...
                                        <div className="pl-4 lg:pl-8 space-y-3 lg:space-y-4">
                                            <div className="flex items-center gap-2 lg:gap-3 group/line">
                                            ...
                                            <div className="pt-4 lg:pt-6">
                                                ...
                                                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-6 pl-2 lg:pl-4">
                                                    {activeFile.content.stack.map((tech) => (
                                                        <div key={tech.name} className="flex flex-col items-center gap-2 lg:gap-3 group/tech cursor-pointer">
                                                            <div className="w-10 h-10 lg:w-14 lg:h-14 rounded-xl lg:rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center p-2.5 lg:p-3.5 group-hover/tech:scale-110 group-hover/tech:border-[#1fa3de]/40 group-hover/tech:bg-[#1fa3de]/10 transition-all duration-500 shadow-xl group-hover/tech:shadow-[#1fa3de]/10">
                                                            ...
                                                            <span className="text-[8px] lg:text-[10px] text-white/50 uppercase tracking-widest font-black group-hover/tech:text-white transition-colors">{tech.name}</span>
                                                        </div>
                                                    ))}
                                                </div>
                                                <div className="mt-4 lg:mt-6 text-[#79c0ff]">]</div>
                                            </div>
                                        </div>
                                        <div className="mt-4 lg:mt-6 text-[#79c0ff]">{"};"}</div>
                                    </div>
